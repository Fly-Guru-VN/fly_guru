...
export async function getMonthlyPayroll(
  ...
  const payees: Payee[] = [
    ...
    //
    // Начальник здесь по той же причине и остаётся без строки долга даже после
    // 04.09.2026: доля 15% за его выходы никому не выплачивается, она просто
    // остаётся в кассе — зарплату самому себе он не платит.
    ...allMechanics.map((u) => ({
      kind: "staff" as const,
      id: u.id,
      name: u.name,
