          ...
          {working.map((s) => (
            <li key={s.id} className="rounded-xl px-3 py-2 odd:bg-line/20">
              ...
              {/* Обе даты рядом: слева «с какого числа работает» (от неё идёт
                  доля с абонементов у новичка), справа увольнение. */}
              <div className="mt-2 flex flex-wrap items-end gap-x-4 gap-y-2">
                ...
                <form action={fireInstructorAction} className="flex items-end gap-1.5">
                  ...
                  <ConfirmSubmit
                    message={`Уволить ${s.name}? Он пропадёт из списков и не сможет войти в кабинет. Занятия, смены и расчёт ЗП за отработанные дни останутся на месте.`}
