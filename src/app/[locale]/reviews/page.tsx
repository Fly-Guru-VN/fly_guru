        ...
        <div className="lg:absolute lg:inset-y-0 lg:right-0 lg:flex lg:w-[52%] lg:items-center">
          <div className="relative w-full">
            ...
            {/* Малый кадр внахлёст. Прижат к НИЗУ большого (bottom-0), а не
                поставлен по координате сверху: обе фотографии кончаются одной
                и той же волной и при общем низе сливаются в одну композицию —
                ровно как на макете. Стоило поднять малый кадр выше, и он
                повисал наклейкой посреди фотографии (так и было в первой
                версии).
                ...
                ⚠️ Эти три числа — 38%, 53% и bottom-0 — зашиты В САМ ФАЙЛ
                hero-small.webp: у него низ обрезан ровно по кромке большого
                кадра и поверх кромки нарисована та же волна, иначе белая
                обводка малого кадра обрывалась поперёк волны прямым срезом
                (так и было до 29.08.2026). Сдвинете или перемасштабируете
                кадр — срез перестанет совпадать с волной, и файл придётся
                печь заново из photo_video/отзывы_hero (littlephoto + wave). */}
            <Image
