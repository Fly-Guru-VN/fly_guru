...
//
// cx/cy — ЦЕНТР плашки в процентах от кадра hero.webp (1262×887). Это центры
// пяти рамок-плейсхолдеров, нарисованных дизайнером прямо в макете
// photo_video/отзывы_hero/hero_rewies.png: каждая ровно 293×94 px. Сами рамки
// из картинки вытравлены скриптом scripts/bake-reviews-hero.mjs — плашки
// встают на их место, а до xl плашек нет, и рамки светились бы на телефоне.
// Пришлют новый макет — прогоните скрипт, он найдёт рамки сам и напечатает
...
// садится на макет с запасом и из-под неё гарантированно ничего не выглядывает.
// Считано от ширины кадра (1262 px), поэтому обе величины — в cqw: и высота
// тоже, иначе при смене пропорции файла плашка поедет.
const CHIP_W = ((293 * 1.05) / 1262) * 100; // 24.38
const CHIP_H = ((94 * 1.05) / 1262) * 100; // 7.82
...
export default function ReviewsPage() {
  ...
  return (
    <>
      {/* ── Первый экран ── */}
      {/* Собран по макету hero_rewies (1262×887 после обрезки полей и мягких кромок):
          ...
          Кадр ПРИЖАТ К ВЕРХУ (items-start), ровно как на тандеме. Стоял
          items-center — и там, где текст в левой колонке выше фотографии
          (1280–1440 px), кадр съезжал вниз и между ним и шапкой зияла белая
          полоса до 31 px. David поймал это на ПК 01.09.2026. Пустая полоса
          ПОД кадром на этих ширинах — так и задумано, у тандема так же:
          низ кадра вырезан волной и переходит в фон страницы.

          min-h на ПК — ровно высота кадра: 52% ширины окна, делённые на
          пропорцию файла 1262/887, то есть 36.6vw (округлено вверх с 36.55 —
          пусть секция будет на волос выше кадра, а не ниже). Кадр лежит absolute и
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-white to-surface-2 lg:flex lg:min-h-[36.6vw] lg:items-center">
        ...
        {/* Кадр и плашки. Фотография — PNG со своими краями: скруглённый левый
            угол и волна снизу. Ни подложек, ни масок ей не нужно, прозрачные
            края показывают фон страницы. Вклеенного малого кадра, который был
            в макете V2, с макета hero_rewies (01.09.2026) больше нет — теперь
            это один снимок с пятью плашками отзывов.
        ...
        {/* right-[-3px] — кадр НАРОЧНО вылезает на 3 px за правый край окна,
            их срезает overflow-hidden у секции. Иначе вдоль правой кромки
            светится бледная ниточка: обёртка ниже объявлена контейнером
            (container-type), а границу такого слоя Chrome сглаживает, и
            крайний пиксель выходил белым на две трети (мерил 01.09.2026 —
            67–80% белого на 1280/1440/1920, после сдвига 0–19%, как у
            тандема, где container-type нет). Три пикселя фотографии при этом
            уходят за кромку окна — кадр туда и так уходит по макету. */}
        <div className="lg:absolute lg:inset-y-0 lg:right-[-3px] lg:flex lg:w-[52%] lg:items-start">
          {/* container-type: плашки с отзывами прибиты к точкам макета и
              меряются в cqw — процентах от ширины ЭТОГО кадра. Без контейнера
              cqw считались бы от окна, и на широких экранах плашки съехали бы
              с фотографии. */}
          <div className="relative w-full [container-type:inline-size]">
            ...
            {/* Пять коротких отзывов плашками поверх кадра — каждая ровно на
                своей рамке-плейсхолдере из макета (координаты и размер живут
                в HERO_CHIPS / CHIP_W / CHIP_H наверху файла). Ставим по
                ЦЕНТРУ рамки со сдвигом на половину себя: плашка на 5% больше
                рамки, и запас должен лечь поровну со всех сторон.

                Всё внутри плашки меряется в cqw, а не в rem: коробка жёстко
                привязана к макету, её ширина едет вместе с кадром, и текст
                фиксированного размера на 1280 вылезал бы за края, а на 2560
                болтался бы буквами в углу.

                Только от xl: от lg кадр ужимается до 52% окна, и на 1024 в
                плашку остаётся 129 px — буквы становятся нечитаемыми. Ничего
              ...
              {HERO_CHIPS.map((c) => (
                <li
                  ...
                  style={{
                    ...
                    width: `${CHIP_W}cqw`,
                    height: `${CHIP_H}cqw`,
                    paddingInline: "1.1cqw",
                    gap: "1cqw",
                    borderRadius: "1.6cqw",
                    boxShadow: "0 1.3cqw 2.8cqw -1.8cqw rgba(15,34,51,0.6)",
                  }}
                >
                  ...
                  <span className="min-w-0">
                    <span className="flex text-accent" aria-hidden style={{ gap: "0.15cqw" }}>
                      {Array.from({ length: c.rating }).map((_, i) => (
                        <IconStar key={i} style={{ width: "1.05cqw", height: "1.05cqw" }} />
                      ))}
                    </span>
                    <span
