...
-- Членство в клубе даёт ОТКАТАННЫЙ абонемент (ответы начальника от 17.09.2026,
-- решение David от 22.09.2026: «откатан» = остаток дошёл до нуля).
--
-- До этого (09.09.2026, коммит 07fd027) членство заводила продажа абонемента.
-- Теперь его заводит само списание — в той же транзакции и под той же
-- блокировкой строки, что и остаток: двойной клик или два инструктора разом
-- второго членства не создадут (у memberships.client_id стоит UNIQUE), а
-- списание без членства невозможно.
--
-- Уже принятых в клуб по старому правилу (13 клиентов) не трогаем — решение
-- David. Вступление датой занятия, на котором кончились минуты.
--
-- Функция та же, что в 0057; добавлен только блок при remaining = 0.
-- create or replace сохраняет выданные в 0057 права (service_role).
...
begin
  ...
  -- Абонемент откатан — клиент в клубе. Уже состоящего не трогаем: дата его
  -- вступления остаётся прежней.
  if remaining = 0 then
    insert into public.memberships (client_id, since)
    values (sub.client_id, p_date::timestamp at time zone 'Asia/Ho_Chi_Minh')
